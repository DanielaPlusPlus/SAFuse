...
from networks.A1005_ResNeXt_cifar_CCAMix_model2_lessChannel_CatZZhat_InferWithWholeStructure import resnext50
...
from losses.supcon_pixelwise_Weighted import SupConLoss
...
def make_print_to_file(path='./'):
    ...
    sys.stdout = Logger(
        fileName + 'A1008_C100_32_3k3RX50_CCAMix_3LossesCatZZhat_InferWholeStructure_WCE_WSCL.log', path=path)
...
parser.add_argument("--START_EPOCH", type=int, default=400)
...
parser.add_argument("--topN_local_ratio", type=float, default=0.764)
parser.add_argument("--loss_gamma_CE", type=float, default=0.8)
parser.add_argument("--loss_gamma_SCL", type=float, default=0.08)
parser.add_argument("--temp", type=float, default=0.7)
parser.add_argument("--N_min",  type=float, default=25)
parser.add_argument("--N_max",  type=float, default=30)
...
name = './Models_out/A1008_C100_32_3k3RX50_CCAMix_3LossesCatZZhat_InferWholeStructure_WCE_WSCL_best.pt'
if len(args.gpu_ids) > 1:
    if args.RESUME:
        net = resnext50(pretrained=False)
        net.fc2 = nn.Linear(2112, args.NUM_CLASS)
...
# criterion_wce = nn.CrossEntropyLoss(reduction='none')

Acc_best = 0.8168
tm = datetime.datetime.now().strftime('T' + '%m%d%H%M')
results_file_name = results_out_dir + tm + 'A1008_C100_32_3k3RX50_CCAMix_3LossesCatZZhat_InferWholeStructure_WCE_WSCL_results.txt'
...
for epoch in range(args.START_EPOCH, args.NB_EPOCH):
    ...
    for batch in tqdm(trainloader):
        ...
        if args.p_binom > 0 and r < args.cutmix_prob:
            ...
            preds, preds_cell_list, weights_local, topN_idx = net(img_mix, local=True, superpixel_map=SuperP_map_batch_list,
            ...
            if len(topN_idx[0]) > 0:
                ...
                weights_SCL_batch_tp = [weights_local[0][idd] for idd in topN_idx[0]]
                weights_SCL_batch_tp = torch.stack(weights_SCL_batch_tp)
            else:
                weights_SCL_batch_tp = torch.ones(np.unique(SuperP_map_batch_list[0]).shape[0]).cuda()
            label_SCL_batch = label_SCL_batch_tp
            weights_SCL_batch = weights_SCL_batch_tp
            ...
            for sp in range(0, images.shape[0]):
                ...
                if len(topN_idx[sp]) > 0:
                    ...
                    weights_cells_current = [weights_local[sp][idd] for idd in topN_idx[sp]]
                    weights_cells_current = torch.stack(weights_cells_current)

                else:
                    weights_cells_current = torch.ones(len(topN_idx[sp])).cuda()


                """un-weighted local CE loss"""
                # loss_cell = criterion_ce(preds_cell_list[sp], target_cells_current).sum()
                # # print(loss_cell)
                # loss_local_ce = loss_local_ce + loss_cell

                """weighted local CE loss"""
                weights_cells_current = [k/weights_cells_current.sum() for k in weights_cells_current] #superpixel weights norm to [0,1] in an image
                weights_cells_current = torch.tensor(weights_cells_current).cuda()
                loss_cell = (criterion_ce(preds_cell_list[sp], target_cells_current) * weights_cells_current).sum()
                # print(loss_cell)
                loss_local_ce = loss_local_ce + loss_cell
                ...
                if sp < (images.shape[0] - 1):
                    ...
                    if len(topN_idx[sp + 1]) > 0:
                        ...
                        weights_cells_next = [weights_local[sp+1][idd] for idd in topN_idx[sp + 1]]
                        weights_cells_next = torch.stack(weights_cells_next)
                    else:
                        weights_cells_next = torch.ones(np.unique(SuperP_map_batch_list[sp + 1]).shape[0]).cuda()
                    ...
                    weights_SCL_batch = torch.cat((weights_SCL_batch, weights_cells_next), dim=0)
                # print(fea_SCL_batch.shape[0], label_SCL_batch.shape[0])

            weights_SCL_batch = [k/weights_SCL_batch.sum() for k in weights_SCL_batch] #superpixel weights norm to [0,1] in a batch
            weights_SCL_batch = torch.tensor(weights_SCL_batch).cuda()
            loss_local_SCL = criterion_SCL(F.normalize(fea_SCL_batch, dim=1), label_SCL_batch, weights_SCL_batch)
            loss = args.loss_gamma_CE * loss_local_ce / images.shape[0] + loss_total2 + args.loss_gamma_SCL * loss_local_SCL
            train_loss_global.append(loss_total2)
            ...
            # print("local_local_loss:", args.loss_gamma_CE * loss_local_ce / images.shape[0])
            # print("local_SCL_loss:", args.loss_gamma_SCL * loss_local_SCL)
            # print("total_CE_loss:", loss_total*0.5)
            # print("total_CE_loss2:", loss_total2*0.5)
        else:
            # compute output
            preds = net(images)
